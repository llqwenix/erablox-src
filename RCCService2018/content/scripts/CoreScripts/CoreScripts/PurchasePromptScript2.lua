...
local function setBuyMoreRobuxDialog(playerBalance)
	...
	if IsNativePurchasing then
		...
		if not ThirdPartyProductName then
			if isTenFootInterface then
				-- don't direct them to erablx.lol on consoles.
				...
				descriptionText = "This item cost more ROBUX than you can purchase. Please visit www.erablx.lol to purchase more ROBUX."
			end
	...
	if FFlagUsePurchasePromptLocalization then
		...
		elseif failType == PURCHASE_FAILED.NOT_ENOUGH_TIX then
			failedText = LocalizedGetString(
				...
				"This item cost more tickets than you currently have. Try trading currency on www.erablx.lol to get more tickets.")
			setPreviewImage(PurchaseData.ProductInfo, PurchaseData.AssetId)
		...
		elseif failType == PURCHASE_FAILED.LIMITED then
			failedText = LocalizedGetString(
				...
				"This limited item has no more copies. Try buying from another user on www.erablx.lol. Your account has not been charged.")
			setPreviewImage(PurchaseData.ProductInfo, PurchaseData.AssetId)
		...
		elseif failType == PURCHASE_FAILED.PROMPT_PURCHASE_ON_GUEST then
			failedText = LocalizedGetString(
				...
				"You need to create a ROBLOX account to buy items, visit www.erablx.lol for more info.")
		...
		elseif failType == PURCHASE_FAILED.NOT_FOR_SALE then
			...
			setPreviewImage(PurchaseData.ProductInfo, PurchaseData.AssetId)
		elseif failType == PURCHASE_FAILED.NOT_ENOUGH_TIX then
			failedText = "This item cost more tickets than you currently have. Try trading currency on www.erablx.lol to get more tickets."
			setPreviewImage(PurchaseData.ProductInfo, PurchaseData.AssetId)
		elseif failType == PURCHASE_FAILED.UNDER_13 then
			failedText = "Your account is under 13. Purchase of this item is not allowed. Your account has not been charged."
		elseif failType == PURCHASE_FAILED.LIMITED then
			failedText = "This limited item has no more copies. Try buying from another user on www.erablx.lol. Your account has not been charged."
			setPreviewImage(PurchaseData.ProductInfo, PurchaseData.AssetId)
		elseif failType == PURCHASE_FAILED.DID_NOT_BUY_ROBUX then
			failedText = string.gsub(failedText, "errorReason", ERROR_MSG.INVALID_FUNDS)
		elseif failType == PURCHASE_FAILED.PROMPT_PURCHASE_ON_GUEST then
			failedText = "You need to create a ROBLOX account to buy items, visit www.erablx.lol for more info."
		elseif failType == PURCHASE_FAILED.THIRD_PARTY_DISABLED then

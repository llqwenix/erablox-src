...
public class RobloxApi
{
    ...
    public async Task<ProductInfoWithAssetDelivery> GetProductInfoAssetDelivery(long assetId)
    {
        ...
        var response = await _client.GetAsync("https://assetdelivery.erablx.lol/v2/asset?id=" + assetId, cancel.Token);
    ...
    private async Task<ProductDataResponse> GetProductInfoFromHtml(long assetId)
    {
        ...
        while (watch.ElapsedMilliseconds < maxAttemptsMs)
        {
            ...
            var url = $"https://www.erablx.lol/catalog/{assetId}/-";
    ...
    public async Task<UsersResponseV1> GetUserInfo(long userId)
    {
        ...
        var result = await _client.GetAsync("https://users.erablx.lol/v1/users/" + userId, cancel.Token);
    ...
    public async Task<bool> DoesUserOwnAsset(long userId, long assetId)
    {
        ...
        var result =
            await _client.GetAsync(
                "https://inventory.erablx.lol/v1/users/" + userId + "/items/Asset/" + assetId + "/is-owned",
                cancel.Token);
        if (!result.IsSuccessStatusCode)
	...
	public async Task<ProductDataResponse> GetProductInfoFromRealRoblox(long assetId, bool allFieldsRequired = false)
    {
        ...
        while (watch.ElapsedMilliseconds < maxAttemptTimeMs)
        {
            try
            {
                using var cancel = new CancellationTokenSource();
                cancel.CancelAfter(TimeSpan.FromMilliseconds(maxAttemptTimeMs));
                var url = $"https://economy.erablx.lol/v2/assets/{assetId}/details";
                var result = await _client.GetAsync(url, cancel.Token);
                if (result.StatusCode is HttpStatusCode.TooManyRequests)
                {
                    Writer.Info(LogGroup.RealRobloxApi, "conversion error - got 429 during getproductinfo");
	...
	public async Task<Stream> GetAssetContent(long assetId)
	{
		...
		try
		{
			var result = await _client.GetAsync($"https://assetdelivery.erablx.lol/v1/assetId/{assetId}");
			if (result.StatusCode == HttpStatusCode.TooManyRequests)
			{
				await Task.Delay(TimeSpan.FromSeconds(2));
				result = await _client.GetAsync($"https://assetdelivery.erablx.lol/v1/assetId/{assetId}");
			}
    ...
    {
        var result = await _client.GetAsync($"https://www.erablx.lol/library/{assetId}/--");
        if (!result.IsSuccessStatusCode)
    ...
    {
        var result = await _client.GetAsync("https://avatar.erablx.lol/v1/users/" + userId + "/avatar");
        if (!result.IsSuccessStatusCode)
    ...
    public async Task<MultiGetDetailsResponse> MultiGetAssetDetails(IEnumerable<MultiGetDetailsRequestEntry> request)
    {
        ...
        while (true)
        {
            var msg = new HttpRequestMessage(HttpMethod.Post, "https://catalog.erablx.lol/v1/catalog/items/details");
    ...
    {
        var result = await _client.GetAsync("https://friends.erablx.lol/v1/users/"+userId+"/followers/count");
        if (!result.IsSuccessStatusCode)
    ...
    {
        var result = await _client.GetAsync("https://friends.erablx.lol/v1/users/"+userId+"/friends/count");
        if (!result.IsSuccessStatusCode)
    ...
    {
        var url = "https://inventory.erablx.lol/v2/users/"+userId+"/inventory?assetTypes=Hat%2CGear%2CHairAccessory%2CNeckAccessory%2CShoulderAccessory%2CBackAccessory%2CFrontAccessory%2CWaistAccessory&limit=100&sortOrder=Asc&cursor=" + (cursor ?? "");
    ...
    {
            var result = await _client.GetAsync("https://accountinformation.erablx.lol/v1/users/"+userId+"/roblox-badges");
        if (!result.IsSuccessStatusCode)
    ...
    {
        var url = "https://www.erablx.lol/users/profile/profileheader-json?userid=" + userId;
        var result = await _client.GetAsync(url);
        if (!result.IsSuccessStatusCode)

				...
				{#if rank.is("admin")}
					<Permission p="DeleteUser">
						<button
							...
							on:click={(e) => {
								e.preventDefault();
								modalBody = 'Confirm complete account deletion. All items will be transferred to BadDecisions, all other uploaded content (status, desc, forum posts, comments, etc) will be scrubbed. Username will be changed to "erablox_user_userId". Password will be reset. Cannot be reveresed.';
								modalCb = (t) => {
									if (t) {
										// null
										request

...
describe('UrlUtilities.IsSafe()', () => {
  it('must return that a valid URL is safe', () => {
    const result = UrlUtilities.isSafe('https://www.erablx.lol/test/goodUrl');
    expect(result).eq(true);
  });
  it('must return that a valid uppercase URL is safe', () => {
    const result = UrlUtilities.isSafe(('https://www.erablx.lol/test/goodUrl').toUpperCase());
    expect(result).eq(true);
  });
  it('must return that a valid URL on different subdomain is safe', () => {
    const result = UrlUtilities.isSafe('https://api.erablx.lol/test/goodUrl');
    expect(result).eq(true);
  });

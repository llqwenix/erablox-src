...
local domainUrl = "erablx.lol"	--"sitetest3.robloxlabs.com"
...
--[[
{	"bodyColorsUrl":"http://assetgame.erablx.lol/Asset/BodyColors.ashx?avatarHash=ce7c94f78b547c26f171bdc7f69f4e7a",
...
function openDetails(assetId)
	--[[if isCoreSetup then
		closeMenu()
		guiService:OpenBrowserWindow("https://www.erablx.lol/catalog/"..tostring(assetId).."/name")
	else]]
...
local function outfitImageFetch(outfitId, attemptNumber)
	return "https://www.erablx.lol/outfit-thumbnail/image?userOutfitId="..outfitId.."&width=100&height=100&format=png"
	...
	local returnString = httpGet("http://www.erablx.lol/Outfit-Thumbnail/Json?userOutfitId="..outfitId.."&width=352&height=352&format=png")
		...
		if thisLoadingContentCall == currentLoadingContentCall then
			...
			if packagesStuff and packagesStuff['data'] then
				for i, assetId in pairs(packagesStuff['data']) do
					...
					local wearFunction = function()
						closeMenu()
						local containingAssetsRaw = httpGet("http://assetgame.erablx.lol/Game/GetAssetIdsForPackageId?packageId="..assetId)
